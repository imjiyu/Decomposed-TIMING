...
def main(
    ...
    if "our_td" in explainers:
        #from attribution.explainers_td_viz import OUR_TD_VIZ as OUR_TD <- trend first버전
...
### attribution 값은 굳이 다시 저장할 필요 없음!
#    if not os.path.exists("./results_viz/"):
#        os.makedirs("./results_viz/")
#    for key in attr.keys():
#        result = attr[key]
#        if isinstance(result, tuple): result = result[0]
#        np.save('./results_viz/{}_{}_{}_result_{}_{}.npy'.format(data, model_type, key, fold, seed), result.detach().cpu().numpy())
