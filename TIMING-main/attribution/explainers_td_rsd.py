...
class OUR_TD:
    ...
    def attribute_trend_residual_segments(
        ...
        # residual = input - trend  ->  중간점 = baseline + residual
        intermediate = (baselines_s + inputs_s - trends).contiguous()

        # Phase 1: residual 먼저 (baseline -> baseline + residual)
        resid_attr = self._ig_phase(
            baselines_s, intermediate, time_mask, fixed_inputs,
            alphas, targets, return_all,
            n_samples, n_alphas, B, T, D, alpha_chunk)

        # Phase 2: trend 나중 (baseline + residual -> input)
        trend_attr = self._ig_phase(
            intermediate, inputs_s, time_mask, fixed_inputs,
            alphas, targets, return_all,
            n_samples, n_alphas, B, T, D, alpha_chunk)
